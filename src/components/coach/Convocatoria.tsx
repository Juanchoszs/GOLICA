...
export function Convocatoria({ coach }: { coach: any }) {
    ...
    const handleSaveConvocatoria = async () => {
        ...
        try {
            // Prepare data
            const playersList = Object.values(lineup).map((p: any) => ({
                id: p.id,
                name: p.name,
                position: p.position,
                isStarter: true
            ...
            let fullDate = matchInfo.date;
            if (matchInfo.date && matchInfo.time) {
                // Combine date and time into a single datetime
                const [hours, minutes] = matchInfo.time.split(':');
                const dateObj = new Date(matchInfo.date);
                dateObj.setHours(parseInt(hours) || 0, parseInt(minutes) || 0, 0, 0);
                fullDate = dateObj.toISOString();
            } else if (matchInfo.date) {
                // If only date is provided, set to noon to avoid timezone issues
                const dateObj = new Date(matchInfo.date);
                dateObj.setHours(12, 0, 0, 0);
                fullDate = dateObj.toISOString();
            }

            // Save to 'convocatorias' table
            const { error } = await supabase.from('convocatorias').insert({
                opponent: matchInfo.opponent,
                date: fullDate,
                time: matchInfo.time || null,
                location: matchInfo.location || null,
                category: matchInfo.category || 'General',
                formation: formationName,
                players: playersList, // storing as JSONB
            ...
            // Reset form
            setMatchInfo({ opponent: '', date: '', time: '', location: '', category: '' });
            setLineup({});
            setBench([]);
        } catch (error) {
    ...
    return (
        <DndContext onDragEnd={handleDragEnd} onDragStart={(e: any) => setDraggedPlayer(e.active.data.current.player)}>
            <div className="flex flex-col lg:flex-row gap-6 pb-10">
                ...
                {/* Right: Player List */}
                <div className="w-full lg:w-80 flex flex-col gap-3 flex-shrink-0 h-full border-l border-border bg-card/50">
